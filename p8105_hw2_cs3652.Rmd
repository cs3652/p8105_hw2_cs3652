...
In this dataset, there are `r count(distinct(brfss_data, locationdesc))` unique locations. Additionally, all states are represented. I know this by counting the number of values for locationabbr (`r brfss_data %>% distinct(locationabbr) %>% count()`) 

including Washington D.C. The state observed the most was `r brfss_data %>% count(locationabbr) %>% arrange(desc(n)) %>% select(locationabbr) %>% head(1)`. In 2002, the median of the "Excellent" response value was `r brfss_data %>% filter(year == "2002", !is.na(excellent)) %>% summarize(median(excellent))`.
